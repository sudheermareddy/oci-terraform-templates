...
sudo apt-get -y install oracle-java8-installer elasticsearch kibana nginx logstash >> $LOG
...
sudo wget https://raw.githubusercontent.com/sysgain/MSOSS/master/scripts/default -O /etc/nginx/sites-available/default >> $LOG
sudo nginx -t >> $LOG
